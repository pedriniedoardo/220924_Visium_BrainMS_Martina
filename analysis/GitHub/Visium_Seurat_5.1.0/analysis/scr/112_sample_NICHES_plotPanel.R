...
LR_sig_pairs <- c("SPP1—CD44","APOE—TREM2","HLA-DPB1—CD4","NAMPT—ITGA5","PSAP—GPR37")
...
lapply(list_range_scaled, function(p){
  ...
  ggsave(plot = p2,paste0("../out/plot/112_NICHES_",pair,"_scaled_panelSigLR.pdf"),width = 20,height = 4)
})
...
lapply(list_range_norm, function(p){
  ...
  p2 <- wrap_plots(list_plot,ncol = 5)
  ggsave(plot = p2,paste0("../out/plot/112_NICHES_",pair,"_norm_panelSigLR.pdf"),width = 20,height = 4)
})
