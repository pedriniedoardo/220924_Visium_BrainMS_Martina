...
pmap(list(list_brain_NICHES,names(list_brain_NICHES)),function(sl,nm){
  ...
  ggsave(plot = plot,paste0("../out/plot/110_NICHES_",nm,"_panelSigLR.pdf"),width = 20,height = 20)
})
...
meta_full_pair %>%
  write_csv("../out/table/111_meta_full_pair_NICHES_scaled.csv")


# focus on one comparision CD44 and SPP1
p1 <- meta_full_pair %>%
  filter(gene %in% c("SPP1—CD44")) %>%
  filter(!is.na(manual_segmentation3)) %>%
  ggplot(aes(x = manual_segmentation3,y = avg_scaled_exp))+
  geom_boxplot(outlier.shape = NA)+
  geom_point(position = position_jitter(width = 0.2,height = 0))+
...
# save also the full table
meta_full_pair_norm %>%
  write_csv("../out/table/111_meta_full_pair_NICHES_norm.csv")
