...
LR_sig <- read_tsv("../data/135_net.up_cellID_SEN_AST.tsv") %>%
  # remove the problematic ligand
  filter(ligand != "PGD2-PTGDS")
...
pmap(list(list_brain_NICHES,names(list_brain_NICHES)),function(sl,nm){
  ...
  ggsave(plot = plot,paste0("../out/plot/110_NICHES_",nm,"_panelSigLR_ASTSenmayo.pdf"),width = 20,height = 20)
})
...
meta_full_pair %>%
  write_csv("../out/table/111_meta_full_pair_NICHES_scaled_ASTSenmayo.csv")
...
# save also the full table
meta_full_pair_norm %>%
  write_csv("../out/table/111_meta_full_pair_NICHES_norm_ASTSenmayo.csv")
