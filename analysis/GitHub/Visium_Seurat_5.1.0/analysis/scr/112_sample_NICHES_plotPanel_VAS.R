...
LR_sig_pairs <- c("MIF—CXCR4","BMP6—BMPR1B")
...
lapply(list_range_scaled, function(p){
  ...
  ggsave(plot = p2,paste0("../out/plot/112_NICHES_",pair,"_scaled_panelSigLR_VAS.pdf"),width = 20,height = 4)
})
...
lapply(list_range_norm, function(p){
  ...
  p2 <- wrap_plots(list_plot,ncol = 5)
  ggsave(plot = p2,paste0("../out/plot/112_NICHES_",pair,"_norm_panelSigLR_VAS.pdf"),width = 20,height = 4)
})
