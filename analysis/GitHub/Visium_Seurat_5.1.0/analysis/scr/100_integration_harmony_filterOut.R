...
# subset only the spots that have more than 50 reads
data.combined.subset <- subset(data.combined.all, subset = nCount_Spatial > 50)
...
# QC
data.combined.subset <- PercentageFeatureSet(data.combined.subset,
                                             pattern = "^MT-|^Mt-|^mt-",
                                             col.name = "percent_mt")
...
data.combined.subset <- SCTransform(data.combined.subset, assay = "Spatial",vars.to.regress = c("nCount_Spatial","percent_mt"),verbose = TRUE) %>%
  RunPCA(verbose = T) %>%
  FindNeighbors(reduction = "pca", dims = 1:30) %>%
  FindClusters(verbose = T,resolution = 0.1) %>%
  RunUMAP(dims = 1:30)

DimPlot(data.combined.subset, group.by = "dataset")

# 5. Run Harmony
# The next few commands are adapted from the Seurat vignette.
data.combined.subset_harmony <- RunHarmony(data.combined.subset, group.by.vars = "dataset") %>%
  RunUMAP(reduction = "harmony", dims = 1:30) %>%
  FindNeighbors(reduction = "harmony", dims = 1:30) %>%
  FindClusters(resolution = seq(from=0.1,to=1,by=0.1),verbose = T)

# save the object
saveRDS(data.combined.subset_harmony, file = "../out/object/100_data.combined.subset_harmony_filterOut.rds")

# plot --------------------------------------------------------------------
# Martina asked to gather the periplaques categories in one.
data.combined.subset_harmony@meta.data$manual_segmentation2 <- 
  data.combined.subset_harmony@meta.data %>%
...
data.combined.subset_harmony@meta.data$manual_segmentation3 <- data.combined.subset_harmony@meta.data %>%
...
id_resolution <- str_subset(colnames(data.combined.subset_harmony@meta.data),pattern = "SCT_snn_res") %>%
  sort()
...
list_plot <- lapply(id_resolution,function(x){
  ...
  # define many colors
  # color_id <- alphabet(length(unique(data.combined.subset_harmony[[x]] %>% unlist())))
  # show_col(color_id)
  # names(color_id) <- unique(data.combined.subset_harmony[[x]] %>% unlist())
  
  # make the plot
  plot <- DimPlot(data.combined.subset_harmony,
                  reduction = "umap",
                  group.by = x,
                  label = F,
                  raster = T)
...
ggsave("../out/plot/100_UMAPCluster_resolutions_filterOut.pdf",width = 25,height = 15)

# decide the res 0.1
color_id <- alphabet(length(unique(data.combined.subset_harmony$dataset)))
show_col(color_id)
names(color_id) <- unique(data.combined.subset_harmony$dataset %>% unlist())

nclust <- data.combined.subset_harmony[["seurat_clusters"]] |> unique() |> nrow()
cluster_cols <- DiscretePalette(nclust, palette = "polychrome")

(DimPlot(data.combined.subset_harmony, reduction = "umap",group.by = "dataset") + scale_color_manual(values = color_id)) +
  DimPlot(data.combined.subset_harmony,
...
DimPlot(data.combined.subset_harmony,
        group.by = "SCT_snn_res.0.2",split.by = "dataset",
        shuffle = TRUE,
...
DimPlot(data.combined.subset_harmony,
        group.by = "SCT_snn_res.0.2",split.by = "class_top",
        shuffle = TRUE,
        cols = cluster_cols,ncol = 4)

DimPlot(data.combined.subset_harmony,
        group.by = "SCT_snn_res.0.2",split.by = "manual_segmentation",
        shuffle = TRUE,
        cols = cluster_cols,ncol = 4)

DimPlot(data.combined.subset_harmony,
        group.by = "SCT_snn_res.0.2",split.by = "manual_segmentation2",
        shuffle = TRUE,
        cols = cluster_cols,ncol = 3)

DimPlot(data.combined.subset_harmony,
        group.by = "SCT_snn_res.0.2",split.by = "manual_segmentation3",
        shuffle = TRUE,
        cols = cluster_cols,ncol = 3)

# plot the number of reads on the UMAP
(FeaturePlot(data.combined.subset_harmony,features = "nCount_Spatial",order = T)+scale_color_viridis_c(option = "turbo",limits = c(0,5000),oob = scales::squish)) +(FeaturePlot(data.combined.subset_harmony,features = "nFeature_Spatial",order = T)+scale_color_viridis_c(option = "turbo",limits = c(0,3000),oob = scales::squish))

# plot as a violin
VlnPlot(data.combined.subset_harmony,features = c("nCount_Spatial"),group.by = "manual_segmentation2")+scale_y_continuous(trans = "log1p",breaks = c(0,1,10,100,1000,10000,100000))

(FeaturePlot(data.combined.subset_harmony,features = "nCount_SCT",order = T)+scale_color_viridis_c(option = "turbo")) +(FeaturePlot(data.combined.subset_harmony,features = "nFeature_SCT",order = T)+scale_color_viridis_c(option = "turbo"))

# top markers -------------------------------------------------------------
DefaultAssay(data.combined.subset_harmony) <- "SCT"
data.combined.subset_harmony2 <- PrepSCTFindMarkers(data.combined.subset_harmony)
...
# ones
Idents(data.combined.subset_harmony) <- "SCT_snn_res.0.2"
total_h.markers <- RunPrestoAll(data.combined.subset_harmony, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)

# save the table of all markers
total_h.markers %>%
  write_tsv("../out/table/100_FindAllMarkers_HarmonySample_res0.2_filterOut.tsv")
...
dittoSeq::dittoDotPlot(data.combined.subset_harmony,
                       vars = unique(top_specific_markers$gene), 
                       group.by = "SCT_snn_res.0.2")+scale_color_viridis_c(option = "turbo",name="relative \nexpression")
ggsave("../out/plot/100_Ditto_HarmonySample_res0.2_MG_subcluster_filterOut.pdf",width = 15,height = 5)

# plot the proportions
df_summary <- data.combined.subset_harmony@meta.data %>%
  group_by(dataset,manual_segmentation2,SCT_snn_res.0.2) %>%
  summarise(n = n()) %>%
  ungroup() %>%
...
df_summary %>%
  ...
  facet_wrap(~SCT_snn_res.0.2,ncol = 1,scales = "free")+
  scale_y_sqrt()
ggsave("../out/plot/100_plot_clusterProp_res0.2_MG_subcluster_filterOut.pdf",height = 20,width = 4)

mat_zscore <- data.combined.subset_harmony@meta.data %>%
  filter(!is.na(manual_segmentation2)) %>%
  group_by(manual_segmentation2,SCT_snn_res.0.2) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  ...
  select(manual_segmentation2,SCT_snn_res.0.2,zscore) %>%
  pivot_wider(names_from = manual_segmentation2,values_from = zscore) %>%
  column_to_rownames(var = "SCT_snn_res.0.2")

Heatmap(mat_zscore)

# do the same also for the manual_segmentation3
# plot the proportions
df_summary2 <- data.combined.subset_harmony@meta.data %>%
  # remove the NA classification
  # filter(!is.na(manual_segmentation3)) %>%
  group_by(dataset,manual_segmentation3,SCT_snn_res.0.2) %>%
  summarise(n = n()) %>%
  ungroup() %>%
...
df_summary2 %>%
  ...
  facet_wrap(~SCT_snn_res.0.2,ncol = 1,scales = "free")+
  scale_y_sqrt()
ggsave("../out/plot/100_plot_clusterProp_res0.2_MG_subcluster_filterOut_manulaSegmentation3.pdf",height = 20,width = 4)

mat_zscore2 <- data.combined.subset_harmony@meta.data %>%
  filter(!is.na(manual_segmentation3)) %>%
  group_by(manual_segmentation3,SCT_snn_res.0.2) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  ...
  group_by(SCT_snn_res.0.2) %>%
  mutate(zscore = (prop-mean(prop))/sd(prop)) %>%
  select(manual_segmentation3,SCT_snn_res.0.2,zscore) %>%
