...
# focus only on a subset of slides
list_brain <- list_brain[c("V01","V02","V12")]
list_brain_bayespace <- list_brain_bayespace[c("V01","V02","V12")]
...
# save the object
saveRDS(data.combined.all_harmony, file = "../out/object/101_data.combined.all_harmony_regressOut_smallerPool.rds")
# data.combined.all_harmony <- readRDS(file = "../out/object/101_data.combined.all_harmony_regressOut_smallerPool.rds")
...
ggsave("../out/plot/101_UMAPCluster_resolutions_regressOut_smallerPool.pdf",width = 25,height = 15)
...
DimPlot(data.combined.all_harmony,
        group.by = "SCT_snn_res.0.5",split.by = "dataset",
        shuffle = TRUE,
...
DimPlot(data.combined.all_harmony,
        group.by = "SCT_snn_res.0.5",split.by = "class_top",
        shuffle = TRUE,
        cols = cluster_cols,ncol = 4)

DimPlot(data.combined.all_harmony,
        group.by = "SCT_snn_res.0.5",split.by = "manual_segmentation",
        shuffle = TRUE,
        cols = cluster_cols,ncol = 4)

DimPlot(data.combined.all_harmony,
        group.by = "SCT_snn_res.0.5",split.by = "manual_segmentation2",
        shuffle = TRUE,
        cols = cluster_cols,ncol = 3)

DimPlot(data.combined.all_harmony,
        group.by = "SCT_snn_res.0.5",split.by = "manual_segmentation3",
        shuffle = TRUE,
        cols = cluster_cols,ncol = 3)
...
# ones
Idents(data.combined.all_harmony) <- "SCT_snn_res.0.5"
...
total_h.markers %>%
  write_tsv("../out/table/101_FindAllMarkers_HarmonySample_res0.5_regressOut_smallerPool.tsv")
...
dittoSeq::dittoDotPlot(data.combined.all_harmony,
                       ...
                       group.by = "SCT_snn_res.0.5")+scale_color_viridis_c(option = "turbo",name="relative \nexpression")
ggsave("../out/plot/101_Ditto_HarmonySample_res0.5_MG_subcluster_regressOut_smallerPool.pdf",width = 15,height = 5)
...
df_test %>%
  ...
  mutate(cell_type = factor(cell_type,levels = c("WM","WM_MT","WM_IMM","EDGE","CORE","CORTEX"))) %>% 
...
df_summary <- data.combined.all_harmony@meta.data %>%
  group_by(dataset,manual_segmentation2,SCT_snn_res.0.5) %>%
  summarise(n = n()) %>%
  ungroup() %>%
...
df_summary %>%
  ...
  facet_wrap(~SCT_snn_res.0.5,scales = "free")+
  scale_y_sqrt()
ggsave("../out/plot/101_plot_clusterProp_res0.5_MG_subcluster_regressOut_smallerPool.pdf",height = 16,width = 16)

mat_zscore <- data.combined.all_harmony@meta.data %>%
  filter(!is.na(manual_segmentation2)) %>%
  group_by(manual_segmentation2,SCT_snn_res.0.5) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  ...
  select(manual_segmentation2,SCT_snn_res.0.5,zscore) %>%
  pivot_wider(names_from = manual_segmentation2,values_from = zscore) %>%
  column_to_rownames(var = "SCT_snn_res.0.5")
...
df_summary2 <- data.combined.all_harmony@meta.data %>%
  ...
  group_by(dataset,manual_segmentation3,SCT_snn_res.0.5) %>%
  summarise(n = n()) %>%
  ungroup() %>%
...
df_summary2 %>%
  ...
  facet_wrap(~SCT_snn_res.0.5,scales = "free")+
  scale_y_sqrt()
ggsave("../out/plot/101_plot_clusterProp_res0.5_MG_subcluster_regressOut_manulaSegmentation3_smallerPool.pdf",height = 8,width = 12)

# try also to split by  manual annotation rather then cluster id
df_summary2 %>%
  ggplot(aes(x=SCT_snn_res.0.5,y=prop))+
  geom_boxplot(outlier.shape = NA)+
  geom_point(position = position_jitter(width = 0.2),shape=1)+
  theme_bw() +
  theme(strip.background = element_blank(), axis.text.x = element_text(hjust = 1,angle = 45)) +
  ...
  scale_y_sqrt()
ggsave("../out/plot/101_plot_clusterProp_res0.5_MG_subcluster_regressOut_manulaSegmentation32_smallerPool.pdf",height = 8,width = 12)


mat_zscore2 <- data.combined.all_harmony@meta.data %>%
  filter(!is.na(manual_segmentation3)) %>%
  group_by(manual_segmentation3,SCT_snn_res.0.5) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  ...
  group_by(SCT_snn_res.0.5) %>%
  mutate(zscore = (prop-mean(prop))/sd(prop)) %>%
  select(manual_segmentation3,SCT_snn_res.0.5,zscore) %>%
...
mat_zscore22 <- data.combined.all_harmony@meta.data %>%
  ...
  mutate(tot = sum(n)) %>%
  mutate(prop = n/tot) %>%
  select(manual_segmentation3,SCT_snn_res.0.5,prop) %>%
...
mat_zscore3 <- data.combined.all_harmony@meta.data %>%
  ...
  mutate(zscore = (prop-mean(prop))/sd(prop)) %>%
  select(manual_segmentation3,SCT_snn_res.0.5,zscore) %>%
  pivot_wider(names_from = SCT_snn_res.0.5,values_from = zscore) %>%
...
# split by cluster id
data.combined.all_harmony@meta.data %>%
  select(SCT_snn_res.0.5,AST,IMM,LYM,NEU,OLIGO,OPC,VAS,manual_segmentation3,dataset) %>%
  pivot_longer(cols = -c(SCT_snn_res.0.5,manual_segmentation3,dataset),names_to = "cell_type",values_to = "score") %>%
  group_by(SCT_snn_res.0.5,cell_type,dataset) %>%
...
data.combined.all_harmony@meta.data %>%
  ...
  group_by(SCT_snn_res.0.5,cell_type,dataset) %>%
  summarise(avg_score = mean(score),.groups = "drop") %>%
  ggplot(aes(x=SCT_snn_res.0.5,y=avg_score))+
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitter(width = 0.2),shape=1) +
...
mat_zscore4 <- data.combined.all_harmony@meta.data %>%
  ...
  select(SCT_snn_res.0.5,AST,IMM,LYM,NEU,OLIGO,OPC,VAS,manual_segmentation3,dataset) %>%
  pivot_longer(cols = -c(SCT_snn_res.0.5,manual_segmentation3,dataset),names_to = "cell_type",values_to = "score") %>%
  group_by(SCT_snn_res.0.5,cell_type) %>%
  summarise(avg_score = mean(score),.groups = "drop") %>%
  group_by(SCT_snn_res.0.5) %>%
  mutate(zscore = (avg_score-mean(avg_score))/sd(avg_score)) %>%
  select(cell_type,SCT_snn_res.0.5,zscore) %>%
...
# sl <- list_brain$V01
# sl2 <- list_brain_bayespace$V01
# sl_name <- "V01"
pmap(list(list_brain,list_brain_bayespace,names(list_brain)), function(sl,sl2,sl_name){
  spot_size <- sl@images$slice1@scale.factors$fiducial/sl@images$slice1@scale.factors$hires
  
  SpatialDimPlot(sl, group.by = "manual_segmentation",label = F,pt.size = spot_size)
  SpatialDimPlot(sl2, group.by = "BayesSpace",label = F,pt.size = spot_size)
  
  meta_01 <- sl@meta.data %>%
    rownames_to_column("barcode") %>%
    select(barcode)
  
  meta_01_bayespace <- sl2@meta.data %>%
    rownames_to_column("barcode") %>%
  ...
  meta_integrated <- data.combined.all_harmony@meta.data %>%
    ...
    filter(sample_id == sl_name)
  
  # join all the info
  meta_01_update <- meta_01 %>%
    left_join(meta_01_bayespace,by = "barcode") %>%
    left_join(meta_integrated,by = "barcode") %>%
    column_to_rownames("barcode")
  
  test <- sl
  test@meta.data <- meta_01_update
  ...
  p31 <- SpatialDimPlot(test, group.by = "SCT_snn_res.0.5",label = F,pt.size = spot_size)
  ...
  Idents(test) <- "SCT_snn_res.0.5"
  p32 <- SpatialDimPlot(test,
                        cells.highlight = CellsByIdentities(object = test),
                        facet.highlight = TRUE,pt.size = spot_size)
  
  # Idents(test) <- "SCT_snn_res.1"
  # SpatialDimPlot(test,
  #                cells.highlight = CellsByIdentities(object = test),
  #                facet.highlight = TRUE,pt.size = spot_size)
  
  (p11 + p12) + plot_annotation("BayesSpace",theme = theme(plot.title = element_text(hjust = 0.5)))
  ggsave(paste0("../out/plot/101_SpatialDimPlot_BayesSpace_smallerPool_",sl_name,".pdf"),width = 25,height = 10)
  (p21 + p22) + plot_annotation("manual segmentation",theme = theme(plot.title = element_text(hjust = 0.5)))
  ggsave(paste0("../out/plot/101_SpatialDimPlot_ManualSeg_smallerPool_",sl_name,".pdf"),width = 25,height = 10)
  (p31 + p32) + plot_annotation("integrated clustering",theme = theme(plot.title = element_text(hjust = 0.5)))
  ggsave(paste0("../out/plot/101_SpatialDimPlot_IntegRes0.5_smallerPool_",sl_name,".pdf"),width = 25,height = 10)
})
