...
tot_scores %>% 
  write_tsv("out/table/gradient_TSPO_martina.tsv")
...
tot_scores_summary %>% 
  write_tsv("out/table/gradient_TSPO_summary_martina.tsv")

# plotting test -----------------------------------------------------------
# load the summary and try to plot different situations
test <- read_tsv("out/table/gradient_TSPO_summary_martina.tsv")
...
ggsave("out/image/test_gradient_TSPO_linePlot_martina.pdf",width = 12,height = 6)
...
ggsave("out/image/test_correaltion_celltype_TSPO_scatterPlot_martina.pdf",width = 18,height = 12)

# try a correlation matrix
mat_corr <- df_full %>%
filter(TSPO == "TSPO_norm") %>%
group_by(tissue,cell_type) %>%
summarise(cor = cor(avg.TSPO,avg.cellType,method = "pearson")) %>%
pivot_wider(names_from = cell_type,values_from = cor) %>%
column_to_rownames("tissue")

pdf("out/image/test_correaltion_celltype_TSPO_hm_martina.pdf",width = 5,height = 3)
Heatmap(mat_corr)
dev.off()
...
mutate(test = map(data,function(x){
  cor.test(x$avg.TSPO,x$avg.cellType) %>% 
  tidy()
})) %>% 
unnest(test) %>% 
ungroup() %>% 
...
# save the full table
write_tsv(mat_corr_pvalue,"out/table/mat_corr_pvalue_gradient_TSPO.tsv")
...
ggsave("out/image/plot_correlation_TSPO_deconvolution_CAL_IMM.pdf", width = 9,height = 6)
