...
list_enhanced <- readRDS("out/object/list_enhanced_ALL2.rds")
...
list_plot <- pmap(list(list_enhanced,names(list_enhanced)),function(x,name){
  ...
  if("TSPO" %in% rownames(x)){
    p <- featurePlot(x, "TSPO")+scale_fill_gradient(low = "#F0F0F0",high = "#F03B20",
                                                     limits = c(0,1),
                                                     oob = scales::squish) +
    ...
    # produce the plot
    p <- featurePlot(combined_sce, "TSPO")+scale_fill_gradient(low = "#F0F0F0",high = "#F03B20",
                                                                limits = c(0,1),
                                                                oob = scales::squish) +
      theme(legend.position = "top") +
      labs(title = name)
...
pdf("out/image/list_panel_enhanced_TSPO.pdf",width = 20,height = 20)
...
pdf("out/image/list_panel_enhanced_TSPO_2.pdf",width = 20,height = 20)
