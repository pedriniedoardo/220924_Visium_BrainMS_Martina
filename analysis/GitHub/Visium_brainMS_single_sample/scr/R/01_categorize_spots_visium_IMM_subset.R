...
file_id2 <- dir(folder2) %>% 
  str_subset("SPOTLight_IMM") %>% 
  str_remove_all(pattern = "_SPOTLight_IMM_WholeSlide.tsv")
...
list_meta_dec <- lapply(file_id2,function(x){
  ...
  df_meta <- read_tsv(paste0("out/table/",x,"_SPOTLight_IMM_WholeSlide.tsv")) %>% 
...
list_brain_spotlight_IMM <- lapply(file_id2,function(x){
  # check the progress
  print(x)
  ...
  df_class_top_IMM <- list_meta_dec[[x]] %>% 
      dplyr::select(barcodes,MG.Homeo:MIMS.iron) %>%
      pivot_longer(names_to = "class_top_IMM",values_to = "score",-barcodes) %>%
      group_by(barcodes) %>%
      arrange(desc(score)) %>%
  ...
  meta_fix <- left_join(list_meta_dec[[x]],df_class_top_IMM,"barcodes") %>%
    # add the new catrgory based on the deconvolution of the IMM class
    mutate(class_top2 = case_when(class_top == "IMM"~class_top_IMM,
                                  T~class_top))
...
saveRDS(list_brain_spotlight_IMM,"out/object/list_brain_spotlight_IMM.rds")
list_brain_spotlight_IMM <- readRDS("out/object/list_brain_spotlight_IMM.rds")
...
lapply(file_id2,function(x){
  ...
  # read in the seurat object -----------------------------------------------
  test <- list_brain_spotlight_IMM[[x]]
  
  # make the different classes as categorical
  # SpatialFeaturePlot(test, features = "IMM") + theme(legend.position = "right")
  pdf(paste0("out/image/10_class_deconvolution_IMMSubset_",x,".pdf"),width = 15,height = 15)
  
  # reorder the levels
  test$class_top2 <- factor(test$class_top2,levels = c("NEU","OLIGO","OPC","AST","VAS","LYM","MG.Homeo","MG.Stress","MIMS.foamy","MIMS.iron"))
  
  Idents(test) <- "class_top2"
  print(SpatialDimPlot(test, cells.highlight = CellsByIdentities(object = test), facet.highlight = TRUE, ncol = 3,alpha = 0.5) +
          plot_annotation(title = "class_top2"))
  dev.off()
})
...
#   # make the different classes as categorical
#   # SpatialFeaturePlot(test, features = "IMM") + theme(legend.position = "right")
#   pdf(paste0("out/image/10_class_deconvolution_IMMSubset_",x,"_2.pdf"),width = 15,height = 15)
# 
#   # reorder the levels
#   test$class_top2 <- factor(test$class_top2,levels = c("NEU","OLIGO","OPC","AST","VAS","LYM","MG.Homeo","MG.Stress","MIMS.foamy","MIMS.iron"))
# 
...
# load the dataset
list_spotlight <- readRDS("out/object/list_brain_spotlight_IMM.rds")
...
df_tot_fix %>% 
  write_tsv("out/table/df_tot_SPOTLight_IMM_sofia.tsv")
...
df_tot_fix %>% 
  ...
  group_by(dataset,type,class_top2) %>% 
  ...
  facet_wrap(~class_top2,scales = "free") +
  theme_bw() +
  theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 90))
ggsave("out/image/summary_SPOTLight_IMM_TypeCellType_dotplot.pdf",width = 10,height = 10)
...
mat_02 <- df_tot_fix %>% 
  ...
  dplyr::select(type,class_top2,norm) %>% 
  pivot_wider(names_from = class_top2,values_from = norm) %>% 
  column_to_rownames("type")
...
#                                                      "CI"="yellow",
#                                                      "RM"="blue")))
pdf("out/image/summary_SPOTLight_IMM_TypeCellType_heatmap.pdf",width = 5,height = 4)
# Heatmap(mat_01_filter,name = "prop",
#         top_annotation = column_ha)
...
df_tot_fix %>% 
  dplyr::select(barcodes,dataset,class_top2,type,sofia_segmentation_short) %>% 
  group_by(dataset,sofia_segmentation_short,class_top2) %>% 
  summarise(count_spot = n()) %>% 
  ungroup() %>% 
  ...
  theme_bw() +
  theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 90))
ggsave("out/image/summary_SPOTLight_IMM_LocationCellType_dotplot.pdf",width = 10,height = 10)
...
mat_12 <- df_tot_fix %>% 
  dplyr::select(barcodes,dataset,class_top2,type,sofia_segmentation_short) %>% 
  group_by(sofia_segmentation_short,class_top2) %>% 
  summarise(count_spot = n()) %>% 
  ungroup() %>% 
  ...
  group_by(class_top2) %>% 
  mutate(norm = (prop-mean(prop))/sd(prop)) %>% 
  dplyr::select(sofia_segmentation_short,class_top2,norm) %>% 
  pivot_wider(names_from = class_top2,values_from = norm) %>% 
...
#                                                      "CI"="yellow",
#                                                      "RM"="blue")))
pdf("out/image/summary_SPOTLight_IMM_LocationCellType_heatmap.pdf",width = 5,height = 4)
# Heatmap(mat_01_filter,name = "prop",
#         top_annotation = column_ha)
...
# saveRDS(df_tot_fix,"out/object/df_tot_fix_IMM_dec.rds")
# 
# mat_122 <- df_tot_fix %>% 
#   dplyr::select(barcodes,dataset,class_top2,type,sofia_segmentation_short) %>% 
#   group_by(sofia_segmentation_short,class_top2) %>% 
...
# library(circlize)
# # pdf("out/image/summary_SPOTLight_IMM_LocationCellType_heatmap.pdf",width = 5,height = 4)
# Heatmap(mat_122,name = "Z score",col = colorRamp2(c(-1, 0, 1), c("blue", "white", "red")))
# # dev.off()
