...
data_dir <- "../../../raw_data/spaceranger_out/14_SP1/outs/"
...
ggsave("out/image/01_ncount_V14.pdf",width = 10,height = 5)
...
ggsave("out/image/02_GOI_V14.pdf",width = 20,height = 20)
...
p1
ggsave("out/image/03_V14_p1.pdf",width = 20,height = 20)

p2
ggsave("out/image/03_V14_p2.pdf",width = 20,height = 20)
...
p1 + p2
ggsave("out/image/04_UMAP_V14.pdf",width = 10,height = 5)
...
ggsave("out/image/05_PanelClusters_V14.pdf",width = 15,height = 15)

SpatialDimPlot(brain, cells.highlight = CellsByIdentities(object = brain, idents = c(0:8)), facet.highlight = TRUE, ncol = 3,alpha = 0.5)
ggsave("out/image/05_PanelClusters2_V14.pdf",width = 15,height = 15)
...
df_res %>% 
  write_tsv("out/table/V14_FindMarkers.tsv")
...
pmap(list(list_res,names(list_res)), function(x,name){
  name_plot <- paste0("out/image/06_TopCluster_V14_",name,".pdf")
...
# ggsave("out/image/06_TopCluster2_V14.pdf",width = 15,height = 15)
...
ggsave(filename = "out/image/07_TopSpatialVariableFeatures_V14.pdf",width = 20,height = 20)

# since the computation is quite intense save the object up to this poing
# brain <- readRDS("../out_large/brain_V14.rds")
saveRDS(brain,"../out_large/brain_V14.rds")
...
# SpatialFeaturePlot(brain, features = c("OLIGO", "OPC", "AST", "IMM", "NEU", "VAS", "LYM"), pt.size.factor = 1.6, ncol = 3, crop = T)
# 
# ggsave("out/image/07_CCA_label_transfer_V14.pdf",width = 15,height = 15)
# 
...
# ggsave("out/image/07_CCA_label_transfer2_V14.pdf",width = 15,height = 15)
# 
