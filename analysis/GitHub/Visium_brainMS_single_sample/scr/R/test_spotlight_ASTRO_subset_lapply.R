...
# load the SP dataset -----------------------------------------------------
# this one was already preprocessed usin the seurat workflow and added the spotlight classification
list_brain_spotlight <- readRDS("out/object/list_brain_all_spotlight.rds")
# test <- list_brain_spotlight$V01

# load the sc reference ---------------------------------------------------
# notice that this one was aready preprocessed
reference <- readRDS("../out_large/ASTRO_ALL_SCTransformed.rds")
DimPlot(reference, group.by = "clusterCellType", label = TRUE)

# run it only on a subset of cells
reference_subset <- subset(reference,clusterCellType %in% c("ASTRO nr","ASTRO reactive","ASTRO sen","AIMS"))
DimPlot(reference_subset, group.by = "clusterCellType", label = TRUE)
...
# Seurat::Idents(reference_subset) <- "clusterCellType"
# cluster_markers_all <- Seurat::FindAllMarkers(object = reference_subset,
#                                               assay = "SCT",
#                                               slot = "data",
#                                               verbose = TRUE,
#                                               only.pos = TRUE)
# 
# saveRDS(object = cluster_markers_all,file = "out/object/reference_ASTRO_ALL_markers.rds")
cluster_markers_all <- readRDS(file = "out/object/reference_ASTRO_ALL_markers.rds")

# for this task focus only on the deconvolution of some annotations
# cluster_markers_all_subset <- cluster_markers_all %>% 
#   filter(cluster %in% c("MG Homeo","MIMS-foamy","MG Stress","MIMS-iron")) %>% 
#   mutate(cluster=as.character(cluster))

# table(cluster_markers_all_subset$cluster)
table(cluster_markers_all$cluster)

# loop the processing -----------------------------------------------------
pmap(list(list_brain_spotlight,names(list_brain_spotlight)),function(test,name){
  print(name)
  # deconvolution of the whole slide ----------------------------------------
  set.seed(123)
  
  spotlight_whole_slide <- spotlight_deconvolution(
    se_sc = reference_subset,
    counts_spatial = test@assays$Spatial@counts,
  ...
  saveRDS(object = spotlight_whole_slide, file = paste0("out/object/SPOTLight_ASTRO_WholeSlide_",name,".rds"))
  
  
  # Read RDS object ---------------------------------------------------------
  # spotlight_whole_slide <- readRDS(file = "out/object/spotlight_IMM_whole_slide.rds")
  
  nmf_mod <- spotlight_whole_slide[[1]]
  decon_mtrx <- spotlight_whole_slide[[2]]
  ...
  rownames(decon_mtrx) <- colnames(test)
  ...
  test@meta.data %>% 
    ...
    write_tsv(paste0("out/table/",name,"_SPOTLight_ASTRO_WholeSlide.tsv"))
...
#   object = test,
#   features = c("MG.Homeo","MG.Stress","MIMS.foamy","MIMS.iron"),
#   alpha = c(0.1, 1))
# # ggsave("out/image/01_spotlight_deconvolution_panel.pdf",width = 15,height = 10)
# 
# # 0.7.2 Spatial scatterpies -----------------------------------------------
# # Plot spot composition of all the spots.
# cell_types_all <- colnames(decon_mtrx)[which(colnames(decon_mtrx) != "res_ss")]
# 
# spatial_scatterpie(se_obj = test,
#                    cell_types_all = c("MG.Homeo","MG.Stress","MIMS.foamy","MIMS.iron"),
#                    img_path = "../../../raw_data/spaceranger_out/01_SP1_novaseq/outs/spatial/tissue_lowres_image.png",
#                    pie_scale = 0.4)
# # ggsave("out/image/01_spotlight_deconvolution_piechart.pdf",width = 5,height = 5)
# 
...
# 
# graph_ntw <- get_spatial_interaction_graph(decon_mtrx = decon_mtrx[,c("MG.Homeo","MG.Stress","MIMS.foamy","MIMS.iron")])
# 
...
# decon_mtrx_sub <- decon_mtrx[, c("MG.Homeo","MG.Stress","MIMS.foamy","MIMS.iron")]
