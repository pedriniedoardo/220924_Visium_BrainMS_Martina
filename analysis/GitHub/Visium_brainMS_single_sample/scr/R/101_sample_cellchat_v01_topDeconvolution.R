# AIM ---------------------------------------------------------------------
# test run the CellChat v2 on slide V1 use the cluster deconvolution as variable
...
# 
# scale.factors <- jsonlite::fromJSON(txt = file.path("../../../raw_data/spaceranger_out/01_SP1_novaseq/outs/spatial/", 'scalefactors_json.json'))
...
# test@images$slice1@scale.factors$spot <- 89.4725

SpatialDimPlot(test, label = T, label.size = 3,group.by = "class_top")
...
color.use <- scPalette(nlevels(test)); names(color.use) <- levels(test)
Seurat::SpatialDimPlot(test, label = T, label.size = 3, cols = color.use)

# Prepare input data for CelChat analysis
# normalized data matrix
data.input <- Seurat::GetAssayData(test, slot = "data", assay = "SCT")
...
meta <- data.frame(labels = Seurat::Idents(test),
                   samples = "sample1",
                   row.names = names(Seurat::Idents(test)))
...
spatial.locs <- Seurat::GetTissueCoordinates(test, scale = NULL, cols = c("imagerow", "imagecol")) 
...
scalefactors <- jsonlite::fromJSON(txt = file.path("../../../raw_data/spaceranger_out/01_SP1_novaseq/outs/spatial/", 'scalefactors_json.json'))
...
ttt <- netAnalysis_contribution(cellchat, signaling = pathways.show)

cellchat@LR %>% 
  data.frame() %>%
  filter(LRsig.pathway_name %in% cellchat@netP$pathways) %>%
  select(LRsig.interaction_name,LRsig.pathway_name,LRsig.ligand,LRsig.receptor) %>%
  filter(LRsig.interaction_name %in% rownames(ttt$data))
...
spatialFeaturePlot(cellchat, features = c("APP","SORL1"), point.size = 0.8, color.heatmap = "Reds", direction = 1)
...
saveRDS(cellchat, file = "out/object/101_cellchat_V01.rds")
