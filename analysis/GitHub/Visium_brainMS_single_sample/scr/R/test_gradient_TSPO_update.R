...
# check if TSPO is expressed or not
df_gene_exp <- lapply(list_brain, function(x){
  data.frame(TSPO_present = "TSPO" %in% rownames(x))
}) %>%
  bind_rows(.id = "slide")

slide_TSPO <- df_gene_exp %>%
  filter(TSPO_present == T) %>%
  pull(slide)

# keep only the slides that have TSPO
int_slides2 <- intersect(int_slides,slide_TSPO)
...
list_out <- lapply(int_slides2,function(x){
  ...
  # add the expression of TSPO. use the normaized espression
  dfTSPO1 <- FetchData(test,"TSPO",slot="data") %>%
    dplyr::rename("TSPO_norm" = "TSPO") %>%
    rownames_to_column("Barcode")
  
  dfTSPO2 <- FetchData(test,"TSPO",slot="count") %>%
    dplyr::rename("TSPO_raw" = "TSPO") %>%
    rownames_to_column("Barcode")
  
  # add the new annotation provide by sofia
  full_meta <- left_join(current_meta,test_meta,"Barcode") %>%
    left_join(dfTSPO1,"Barcode") %>%
    left_join(dfTSPO2,"Barcode")
  dim(full_meta)
  
  # add the metadata to the original object. it is just one column
  test$manual_segmentation <- full_meta$manual_segmentation
  test$TSPO_raw <- full_meta$TSPO_raw
  test$TSPO_norm <- full_meta$TSPO_norm
  ...
  df_plot <- full_meta %>% 
    dplyr::select(Barcode,manual_segmentation,TSPO_norm,TSPO_raw) %>% 
    pivot_longer(names_to = "TSPO",values_to = "exp",-c(Barcode,manual_segmentation))
...
tot_scores %>% 
  write_tsv("out/table/gradient_TSPO_martina_update.tsv")

# make the summary of the gradients
tot_scores_summary <- tot_scores %>% 
  group_by(slide,manual_segmentation,TSPO) %>% 
  summarise(avg = mean(exp),
            sd = sd(exp),
            med = median(exp))

tot_scores_summary %>% 
  write_tsv("out/table/gradient_TSPO_summary_martina_update.tsv")

# plotting test -----------------------------------------------------------
# load the summary and try to plot different situations
test <- read_tsv("out/table/gradient_TSPO_summary_martina_update.tsv")
...
df_test %>%
  ggplot(aes(x=area,y=avg,group=slide))+geom_line()+facet_grid(TSPO~tissue,scales = "free")+theme_bw()+theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
ggsave("out/image/test_gradient_TSPO_linePlot_martina_update.pdf",width = 12,height = 6)

####
# is there any correlation between the TSPO avg expressino in the different areas and the cell type deconvulution?
...
ggsave("out/image/test_lineplot_celltype_TSPO_martina_update.pdf",width = 12,height = 12)

# test a correlation approach
df_full <- left_join(df_test,df_test_cellType,by=c("slide","manual_segmentation","condition","annotation","tissue_class","tissue"),suffix = c(".TSPO",".cellType")) %>%
  filter(!slide %in% c("V07"))

# save the table with CHIT expression and prop of different cell type
df_full %>%
  write_tsv("out/table/df_full_gradient_TSPO_update.tsv")

# plot the correlations
df_full %>%
  filter(TSPO == "TSPO_norm") %>%
  ggplot(aes(x=avg.TSPO,y=avg.cellType))+geom_point()+facet_wrap(tissue~cell_type,scales = "free",ncol=7)+geom_smooth(method = "lm")+theme_bw()+theme(strip.background = element_blank())
ggsave("out/image/test_correaltion_celltype_TSPO_scatterPlot_martina_update.pdf",width = 18,height = 12)

# try a correlation matrix
mat_corr <- df_full %>%
  filter(TSPO == "TSPO_norm") %>%
  group_by(tissue,cell_type) %>%
  summarise(cor = cor(avg.TSPO,avg.cellType,method = "spearman")) %>%
  pivot_wider(names_from = cell_type,values_from = cor) %>%
  column_to_rownames("tissue")

pdf("out/image/test_correaltion_celltype_TSPO_hm_martina_update.pdf",width = 5,height = 3)
Heatmap(mat_corr)
dev.off()
...
mat_corr_pvalue <- df_full %>%
  ...
  mutate(test = map(data,function(x){
    cor.test(x$avg.TSPO,x$avg.cellType,method = "spearman") %>% 
      tidy()
  })) %>% 
  unnest(test) %>% 
  ungroup() %>% 
...
# save the full table
write_tsv(mat_corr_pvalue,"out/table/mat_corr_pvalue_gradient_TSPO_update.tsv")

# try another option for the plotting
# try to show the trend side by side of the deconvolution and the TSPO expression in CAL samples
p1 <- df_test_cellType %>% 
...
p3 <- df_full %>%
  ...
  filter(tissue %in% c("CAL"),
         cell_type %in% c("IMM")) %>% 
  ggplot(aes(x=avg.TSPO,y=avg.cellType))+geom_point()+
...
ggsave("out/image/plot_correlation_TSPO_deconvolution_CAL_IMM_update.pdf", width = 9,height = 6)
