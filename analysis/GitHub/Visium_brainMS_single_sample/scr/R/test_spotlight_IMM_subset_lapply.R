...
reference <- readRDS("../out_large/IMM_ALL_SCTransformed.rds")
DimPlot(reference, group.by = "clusterCellType", label = TRUE)

# run it only on a subset of cells
reference_subset <- subset(reference,clusterCellType %in% c("MG Homeo","MIMS-foamy","MG Stress","MIMS-iron"))
...
#                                               only.pos = TRUE)
# 
# saveRDS(object = cluster_markers_all,file = "out/object/reference_IMM_ALL_markers.rds")
cluster_markers_all <- readRDS(file = "out/object/reference_IMM_ALL_markers.rds")
...
pmap(list(list_brain_spotlight,names(list_brain_spotlight)),function(test,name){
  ...
  saveRDS(object = spotlight_whole_slide, file = paste0("out/object/SPOTLight_IMM_WholeSlide_",name,".rds"))
  ...
  test@meta.data %>% 
    ...
    write_tsv(paste0("out/table/",name,"_SPOTLight_IMM_WholeSlide.tsv"))
